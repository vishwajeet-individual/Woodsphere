      ...
      <Container maxWidth="lg">
        ...
        {orders.length === 0 ? (
          <Box textAlign="center" py={10}>
             ...
             <Button component={Link} href="/search" variant="contained" sx={{ mt: 2, borderRadius: 50 }}>
                Start Shopping
             </Button>
          </Box>
        ) : (
          <Stack spacing={3}>
            {orders.map((order) => (
              <Card key={order.id} variant="outlined" sx={{ p: 3, borderRadius: 3 }}>
                ...
                <Box mt={2}>
                   <Chip label={order.status} size="small" color={order.status === 'DELIVERED' ? 'success' : 'primary'} />
