...
// NAV_TREE now uses icon names (string) instead of JSX elements
const NAV_TREE = [
  {
    label: 'Living',
    slug: 'living-room',
  ...
  },
  {
    ...
    subs: [
      ...
      { label: 'Bedside Tables', slug: 'bedside-tables' },
    ],
  },
  {
    label: 'Dining',
  ...
  },
  {
    ...
    subs: [
      ...
      { label: 'Study Tables', slug: 'study-tables' },
    ],
  },
  {
    label: 'Kids',
    ...
    icon: 'ChildCare',
    subs: [
      { label: 'Kids Beds', slug: 'kids-beds-storage' },
      { label: 'Study for Kids', slug: 'study-for-kids' },
    ],
  },
  {
    label: 'Décor',
    ...
    subs: [
      ...
      { label: 'Wall Decor', slug: 'wall-decor-mirrors' },
    ],
  },
  {
    label: 'More',
...
// Footer links under the Sign Out / Sign In button
const FOOTER_LINKS = [
...
export default function Header({ settings }: HeaderProps) {
  ...
  useEffect(() => {
    if (isSearchOpen && desktopSearchRef.current) {
      setTimeout(() => desktopSearchRef.current?.focus(), 100);
    }
  }, [isSearchOpen]);
  ...
  const handleMobileExpand = (slug: string) =>
    setMobileExpanded(mobileExpanded === slug ? null : slug);
  ...
  // Sub-menu link resolver
  const getSubLink = (parentSlug: string, sub: any) =>
    sub.href || `/search?category=${parentSlug}&sub=${sub.slug}`;

  // Category link resolver
  const getCategoryLink = (slug: string) =>
    slug === 'sale' ? '/search?sale=true' : `/category/${slug}`;
  ...
  return (
    <>
          ...
          <Toolbar disableGutters sx={{ justifyContent: 'space-between', height: 64 }}>
            ...
            >
              {navTree.map((item: any) => (
                <Box
                  key={item.slug}
                  onMouseEnter={() => setHoveredMenu(item.slug)}
                  sx={{
                    position: 'relative',
                    height: '100%',
                    display: 'flex',
              ...
              <Box sx={{ display: { xs: 'none', sm: 'block' } }}>
                {isSearchOpen ? (
                  ...
                  >
                    ...
                    />
                    <IconButton size="small" onClick={() => setIsSearchOpen(false)}>
      ...
      <Drawer
        ...
        PaperProps={{
          sx: { width: '85%', maxWidth: 320, bgcolor: '#ffffff', overflow: 'visible' },
        }}
      >
        {mobileOpen && (
          <IconButton
              ...
              <ListItem disablePadding sx={{ mb: 0.5 }}>
                ...
                >
                  <Box
                    sx={{
                      mr: 2,
                      color: item.isSale ? 'inherit' : 'text.secondary',
                      display: 'flex',
                    }}
                  >
                    {getIcon(item.icon)}
              ...
              {!item.isSale && (
                <Collapse in={mobileExpanded === item.slug} timeout="auto" unmountOnExit>
                  <List
                    component="div"
                    disablePadding
                    sx={{ pl: 2, ml: 2, borderLeft: '2px solid #f0f0f0' }}
                  >
                    {item.subs?.map((sub: any) => (
                      <ListItemButton
                        key={sub.slug}
        ...
        {/* FOOTER: Auth button + utility links */}
        <Box
          sx={{
            p: 3,
            bgcolor: '#f9fafb',
            mt: 'auto',
            textAlign: 'center', // Align everything center visually
          }}
        >
          {!session ? (
            <Button
              ...
              variant="contained"
              size="large"
              sx={{ borderRadius: 2, py: 1.5, bgcolor: '#1d1d1f' }}
              onClick={() => setMobileOpen(false)}
            >
              Sign In / Register
            </Button>
          ...
          <Box sx={{ mt: 1.5 }}>
            ...
            {/* INLINE, perfectly centered responsive layout */}
            <Stack
              direction="row"
              spacing={1.5}
